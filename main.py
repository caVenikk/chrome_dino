...
def SaveScore():
    ...
    f = open('score.txt', 'r')
    lines = f.readlines()
    lines.append(f'{name} {gameState.count} {now.day}.{now.month}.{now.year} {now.hour}:{now.minute}\n')
    f.close()
    f = open('score.txt', 'w')
    f.writelines(lines)
    f.close()
...
def button(screen, position, text):
    ...
    pygame.draw.line(screen, (150, 150, 150), (text_rect.x, text_rect.y - 2), (text_rect.x, text_rect.y + text_rect.h), 5)
    pygame.draw.line(screen, (50, 50, 50), (text_rect.x, text_rect.y + text_rect.h), (text_rect.x + 125, text_rect.y + text_rect.h), 5)
    pygame.draw.line(screen, (50, 50, 50), (text_rect.x + 125, text_rect.y + text_rect.h), [text_rect.x + 125, text_rect.y], 5)
...
def scoreboard_menu():
    ...
    if 0 < len(data) < 5:
        for i in range(len(data)):
            players.append(font.render(f"{data[i]['name']} — {data[i]['score']} ({data[i]['date']} {data[i]['time']})", True, [83, 83, 83]))
    elif len(data) <= 0:
